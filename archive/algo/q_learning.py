...
class DQN(base.ValueNet):
    ...
    def flush_buffer(self, **kwargs):
        """Add experience tuples to the replay buffer."""

        self.replay_buffer.push(**kwargs)
        
    def train(self, cuda):
        """Train DDQN using batches from the replay buffer"""
...
class MFQ(base.ValueNet):
    ...
    def train(self, cuda):
        ...
        for i in range(batch_name):
            ...
            feat_next = torch.FloatTensor(feat_next)
            acts = torch.LongTensor(acts)
            act_prob = torch.FloatTensor(act_prob)
            act_prob_next = torch.FloatTensor(act_prob_next)
            rewards = torch.FloatTensor(rewards)
            dones = torch.FloatTensor(dones)
            masks = torch.FloatTensor(masks)
