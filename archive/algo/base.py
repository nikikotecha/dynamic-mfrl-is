...
class ValueNet(nn.Module):
    def __init__(self, env, name, handle, update_every=5, use_mf=False, learning_rate=1e-4, tau=0.005, gamma=0.95):
        ...
        self.view_space  = env.view_space[handle]
    ...
    def _construct_net(self):
        ...
        #TODO: Change this so its not conv2d? 

        #temp_dict['conv1'] = nn.Conv2d(in_channels=self.view_space[1], out_channels=32, kernel_size=3) #CHANGE to -1 from view_space[2]
        #temp_dict['conv2'] = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3)
        temp_dict['obs_linear'] = nn.Linear(self.view_space, 128)
    ...
    def calc_target_q(self, obs, feature, dones, rewards, prob=None):
        t_h = torch.relu(self.target_net['obs_linear'](obs))
        t_h = torch.cat([t_h, self.target_net['emb_linear'](feature)], -1)

        if self.use_mf:
            t_h = torch.cat([t_h, self.target_net['prob_emb_linear'](prob)], -1)
        t_q = self.target_net['final_linear'](t_h)

        e_h = torch.relu(self.eval_net['obs_linear'](obs))
        e_h = torch.cat([e_h, self.eval_net['emb_linear'](feature)], -1)
        if self.use_mf:
    ...
    def act(self, obs, feature, prob=None, eps=None):
        ...
        #e_h = F.relu(self.eval_net['obs_linear'](F.relu(self.eval_net['conv1'](obs)))).flatten(start_dim=1)
        ...
        e_h = F.relu(self.eval_net['obs_linear'](obs))
        e_h = torch.cat([e_h, self.eval_net['emb_linear'](feature)], -1)
        if self.use_mf:
            e_h = torch.cat([e_h, self.eval_net['prob_emb_linear'](prob)], -1)
        e_q = self.eval_net['final_linear'](e_h)
